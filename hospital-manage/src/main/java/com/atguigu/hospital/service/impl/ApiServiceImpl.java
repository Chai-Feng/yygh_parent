...
public class ApiServiceImpl implements ApiService {
    ...
    //    System.out.println("检测+是否存在 === "+jsonObject.getString("logoData"));

        JSONObject bookingRule = jsonObject.getJSONObject("bookingRule");
        paramMap.put("bookingRule",bookingRule.toJSONString());

        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
       paramMap.put("sign", HttpRequestHelper.getSign(paramMap, this.getSignKey())); //加密

       // paramMap.put("sign", MD5.encrypt(this.getSignKey()));  //加密
